...
type SubProblem = { id: string; text: string; userCreated?: boolean; businessId: string };
...
function AddNodeModal({ onClose, onAdd }: { onClose: () => void; onAdd: (text: string) => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.35)' }}
      ...
      <div className="w-full max-w-sm mx-4 rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-zinc-900">Add node directly</h2>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input
            ...
            autoFocus
          />
          <p className="text-xs text-zinc-400">Creates a new node in active state and immediately runs it through the analysis pipeline.</p>
...
function buildHighlightSegments(text: string, ranges: { start: number; end: number }[]) {
  ...
  for (const r of merged) {
    ...
    if (r.start < r.end) segs.push({ text: text.slice(r.start, r.end), highlighted: true });
    pos = r.end;
  }
...
function HighlightableText({ nodeKey, text, highlights, onHighlight, className }: {
  nodeKey: string; text: string;
  ...
  function handleMouseUp() {
    ...
    if (!sel || sel.isCollapsed || !containerRef.current) { setBtnPos(null); return; }
    const range = sel.getRangeAt(0);
    if (!containerRef.current.contains(range.commonAncestorContainer)) { setBtnPos(null); return; }
    ...
    if (end <= start) { setBtnPos(null); return; }
  ...
  useEffect(() => {
    function dismiss(e: MouseEvent) {
      if (containerRef.current && !containerRef.current.contains(e.target as Node)) {
        setBtnPos(null); pendingRef.current = null;
      }
    }
  ...
  return (
    <>
      <div ref={containerRef} onMouseUp={handleMouseUp} className={className} style={{ userSelect: 'text', cursor: 'text' }}>
        {segments.map((seg, i) =>
          seg.highlighted
            ? <mark key={i} style={{ background: '#fef08a', borderRadius: 2, padding: '0 1px' }}>{seg.text}</mark>
      ...
      {btnPos && (
        <button
          ...
          style={{ position: 'fixed', left: btnPos.x, top: btnPos.y, transform: 'translateX(-50%)', zIndex: 9999 }}
...
function DetailPanel({ selectedId, businesses, subproblems, concepts, paperGroups, recommendations, onClose }: {
  selectedId: string | null; businesses: Business[]; subproblems: SubProblem[]; concepts: Concept[];
  ...
  const typeLabel = isBusiness ? 'Business Problem' : displayPaper ? 'Paper' : sp ? (sp.userCreated ? 'Added Node' : 'Sub-problem') : concept ? 'Concept' : null;
  ...
  return (
    <div className="w-64 shrink-0 border-r border-zinc-200 bg-white h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {isBusiness && (
          <HighlightableText nodeKey="business-problem" text={problem}
            highlights={highlights['business-problem'] ?? []} onHighlight={addHighlight}
        ...
        {displayPaper && (
          <div className="space-y-3">
            ...
            <div>
              <HighlightableText nodeKey={`${selectedId}-title`} text={displayPaper.title}
                highlights={highlights[`${selectedId}-title`] ?? []} onHighlight={addHighlight}
            ...
            {displayPaper.abstract && (
              <div>
                ...
                <HighlightableText nodeKey={`${selectedId}-abstract`} text={displayPaper.abstract}
                  highlights={highlights[`${selectedId}-abstract`] ?? []} onHighlight={addHighlight}
        ...
        {sp && !displayPaper && (() => {
          ...
          return (
            <div className="space-y-3">
              <HighlightableText nodeKey={`${selectedId}-text`} text={sp.text}
                highlights={highlights[`${selectedId}-text`] ?? []} onHighlight={addHighlight}
                className="text-sm text-zinc-900 leading-relaxed" />
              {spConcept && <>
                <div>
        ...
        {concept && !displayPaper && (
          <div className="space-y-3">
            {spForConcept && (
              <HighlightableText nodeKey={`${selectedId}-sptext`} text={`"${spForConcept.text}"`}
                highlights={highlights[`${selectedId}-sptext`] ?? []} onHighlight={addHighlight}
            ...
            <div>
              ...
              <HighlightableText nodeKey={`${selectedId}-query`} text={concept.academic_query}
                highlights={highlights[`${selectedId}-query`] ?? []} onHighlight={addHighlight}
...
function NodeDeleteBtn({ onDelete, dark = false }: { onDelete: () => void; dark?: boolean }) {
  return (
    <button
      onClick={e => { e.stopPropagation(); onDelete(); }}
      title="Remove"
      style={{
        position: 'absolute', top: 3, right: 3, width: 15, height: 15, padding: 0,
  ...
  return (
    <>
      <div style={{
        ...
        {d.phase === 'decomposing' && (
          <p style={{ fontSize: 9, opacity: 0.55, marginTop: 4, margin: 0 }}>Decomposing…</p>
        )}
      </div>
...
function DomainCandidateNode({ data }: NodeProps<RFNode<GNodeData>>) {
  ...
  return (
    <>
      <Handle type="target" position={Position.Top}
        style={{ background: '#5eead4', border: '2px solid #fff', width: 8, height: 8, top: -4 }} />
      <div style={{
        position: 'relative',
        ...
        {d.onDelete && <NodeDeleteBtn onDelete={d.onDelete} />}
        <div style={{ fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', opacity: 0.55, marginBottom: 3, fontFamily: 'var(--mono)' }}>Added</div>
        <p style={{ fontSize: 11, fontWeight: 500, lineHeight: 1.35, margin: 0 }}>{d.label}</p>
      </div>
      <Handle type="source" position={Position.Bottom}
        style={{ background: '#5eead4', border: '2px solid #fff', width: 8, height: 8, bottom: -4 }} />
    </>
  );
}
...
function AddNode({ data }: NodeProps<RFNode<GNodeData>>) {
  ...
  return (
    <>
      <Handle type="target" position={Position.Top}
        style={{ background: '#c7d2fe', border: '2px solid #fff', width: 8, height: 8, top: -4, opacity: 0 }} />
      <div
        ...
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#c7d2fe'; (e.currentTarget as HTMLElement).style.color = '#818cf8'; }}
      >
...
function FitOnStructureChange({ nodeCount, businessCount, latestBusinessNodeId, nodes }: {
  nodeCount: number; businessCount: number;
    ...
    if (newBizAdded && latestBusinessNodeId) {
      ...
      const t = setTimeout(() => {
        ...
        if (node) {
          ...
          setCenter(node.position.x + dim.w / 2, node.position.y + dim.h / 2, { zoom: 1.2, duration: 450 });
        } else {
...
function NodeClickPanner({ nodeId, nodes }: { nodeId: string | null; nodes: RFNode<GNodeData>[] }) {
  ...
  useEffect(() => {
    ...
    setCenter(node.position.x + dim.w / 2, node.position.y + dim.h / 2, { zoom: 1.3, duration: 350 });
  }, [nodeId]); // eslint-disable-line react-hooks/exhaustive-deps
  return null;
}

function GraphPanel({
  businesses, subproblems, concepts, paperGroups, recommendations,
  selectedId, onSelect, activeMsgId, onTransformSp, onDeleteNode, onAddNodeForBusiness,
}: {
  businesses: Business[]; subproblems: SubProblem[]; concepts: Concept[];
  paperGroups: Record<string, Paper[]>; recommendations: Recommendation[];
  selectedId: string | null; onSelect: (id: string | null) => void;
  activeMsgId?: string; onTransformSp?: (msgId: string, sp: SubProblem) => Promise<Concept>;
  ...
  const handleTransformFromGraph = useCallback(async (msgId: string, sp: SubProblem) => {
    if (!onTransformSp || graphTransformingId === sp.id) return;
    setGraphTransformingId(sp.id);
    try { await onTransformSp(msgId, sp); }
    finally { setGraphTransformingId(null); }
  }, [onTransformSp, graphTransformingId]);

  useEffect(() => {
    if (businesses.length === 0) { setRfNodes([]); setRfEdges([]); return; }
    ...
    for (const biz of businesses) {
      const bizSubproblems = subproblems.filter(sp => sp.businessId === biz.id);
      const bizConcepts = concepts.filter(c => bizSubproblems.some(sp => sp.id === c.subproblemId));
      const bizPaperGroups = Object.fromEntries(
        Object.entries(paperGroups).filter(([spId]) => bizSubproblems.some(sp => sp.id === spId))
      );
      const addNodeId = `add-node-${biz.id}`;
      const conceptIds = new Set(bizConcepts.map(c => c.subproblemId));
      const existingPaperIds = new Set(Object.values(bizPaperGroups).flat().map(p => p.paperId));
      ...
      if (bizSubproblems.length === 0) {
        ...
        allFinalNodes.push({
          ...
          data: {
            label: biz.problem.slice(0, 50) + (biz.problem.length > 50 ? '…' : ''),
            isSelected: selectedId === biz.id,
            phase: biz.phase,
            onDelete: onDeleteNode ? () => onDeleteNode(biz.id, 'business') : undefined,
          },
        });
        xOffset += 260;
        continue;
      ...
      const structNodes: { id: string; type: string }[] = [{ id: biz.id, type: 'business' }];
      const structEdges: { id: string; source: string; target: string; animated?: boolean; dashed?: boolean }[] = [];

      bizSubproblems.forEach(sp => {
        structNodes.push({ id: sp.id, type: sp.userCreated ? 'domaincandidate' : 'subproblem' });
        structEdges.push({ id: `b-${sp.id}`, source: biz.id, target: sp.id });
      });
      structNodes.push({ id: addNodeId, type: 'add-node' });
      structEdges.push({ id: `b-${addNodeId}`, source: biz.id, target: addNodeId, dashed: true });
      ...
      const allPaperNodeIds = Object.values(bizPaperGroups).flat().map(p => p.paperId);
      recommendations.slice(0, 5).forEach(r => {
        if (!existingPaperIds.has(r.paperId)) {
          ...
          if (allPaperNodeIds.length > 0) {
            structEdges.push({ id: `rec-${biz.id}-${r.paperId}`, source: allPaperNodeIds[0], target: r.paperId, animated: true });
          }
        }
      });
      ...
      bizSubproblems.forEach(sp => {
        ...
        allFinalNodes.push({
          id: sp.id, type: sp.userCreated ? 'domaincandidate' : 'subproblem',
          ...
          data: {
            ...
            onDelete: onDeleteNode ? () => onDeleteNode(sp.id, 'subproblem') : undefined,
          },
        });
      });
      ...
      bizConcepts.forEach(c => {
        ...
        allFinalNodes.push({
          ...
          data: {
            ...
            onDelete: onDeleteNode ? () => onDeleteNode('c-' + c.subproblemId, 'concept') : undefined,
          },
        });
      });
        ...
        papers.slice(0, 3).forEach(p => {
          ...
          allFinalNodes.push({
            ...
            data: {
              ...
              onDelete: onDeleteNode ? () => onDeleteNode(p.paperId, 'paper') : undefined,
            },
          });
        });
      });
        ...
        if (!existingPaperIds.has(r.paperId)) {
          ...
          allFinalNodes.push({
            ...
            data: {
              ...
              onDelete: onDeleteNode ? () => onDeleteNode(r.paperId, 'recommend') : undefined,
            },
          });
        }
      });
      ...
      allFinalNodes.push({
        id: addNodeId, type: 'add-node',
        position: { x: addPos.x + xOffset, y: addPos.y },
        data: { label: '+ Add directly', isSelected: false, onAdd: onAddNodeForBusiness ? () => onAddNodeForBusiness(biz.id) : undefined },
      });
      ...
      structEdges.forEach(e => {
        ...
        allFinalEdges.push({
          ...
          style: {
            stroke: highlight ? '#f59e0b' : isAddEdge ? '#c7d2fe' : isRecEdge ? '#86efac' : isConceptEdge ? '#a78bfa' : e.animated ? '#c7d2fe' : '#ddd6fe',
  ...
  }, [businesses, subproblems, concepts, paperGroups, recommendations, selectedId, graphTransformingId, onTransformSp, onDeleteNode, onAddNodeForBusiness, handleTransformFromGraph]);
  ...
  return (
    <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
      <ReactFlow
        ...
        onNodeClick={(_, node) => { if (node.id.startsWith('add-node-')) return; onSelect(node.id === selectedId ? null : node.id); }}
      ...
      >
        ...
        <FitOnStructureChange
          ...
          latestBusinessNodeId={businesses.length > 0 ? businesses[businesses.length - 1].id : null}
          nodes={rfNodes}
        />
...
function AssistantMessage({ msg, selectedPaperId, onSelectPaper, onTransformOne, onSearchOne }: {
  ...
  void selectedPaperId; void onSelectPaper;
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-3 flex-wrap">
        ...
        {isIngesting && <PhaseTag label={msg.phase === 'ingesting' ? 'Saving…' : 'Computing PPR…'} active />}
        {msgRecs.length > 0 && <PhaseTag label="PPR" done />}
      </div>

      <p className="text-xs text-zinc-500">{sps.length} sub-problem{sps.length !== 1 ? 's' : ''} identified</p>
        ...
        {sps.map((sp, i) => {
          ...
          return (
            <div key={sp.id} className="flex items-start gap-2">
              <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-indigo-600 text-[10px] font-bold text-white mt-0.5">{i + 1}</span>
              <div className="flex-1 min-w-0">
                ...
                {!concept ? (
                  ...
                  <p className="mt-0.5 text-[11px] text-green-600">{papers.length} paper{papers.length !== 1 ? 's' : ''} found</p>
                )}
              </div>
            </div>
          );
      ...
      {isIngesting && (
        <div className="flex items-center gap-2 py-1 text-xs text-zinc-400">
          ...
          <img src="/lemon.png" alt="" style={{ width: 12, height: 12, objectFit: 'contain', animation: 'spin 1.5s linear infinite' }} />
      ...
      {msgRecs.length > 0 && (
        <p className="text-xs text-green-700 flex items-center gap-1.5">
          ...
          PPR complete — {msgRecs.length} recommendation{msgRecs.length !== 1 ? 's' : ''}
        </p>
      )}
    </div>
  );
    ...
    <div className="relative flex flex-col w-80 shrink-0 border-l border-zinc-200 bg-white h-full">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
        {(() => {
          ...
          let cur: { user?: Message; assistant?: Message; key: string } = { key: '' };
            ...
            <div key={pair.key} className="space-y-2">
              {pair.user && (
                <div className="flex justify-end">
                  <div className="max-w-[88%] rounded-2xl rounded-tr-sm bg-indigo-600 px-3.5 py-2.5 text-sm text-white leading-snug">{pair.user.problem}</div>
                </div>
              )}
              {pair.assistant && (
                <AssistantMessage msg={pair.assistant} selectedPaperId={selectedId} onSelectPaper={onSelectNode}
...
export default function Explore() {
  ...
  function reset() {
    setInput(''); setMessages([]); setBusinesses([]);
    setSubproblems([]); setConcepts([]); setPaperGroups({}); setRecommendations([]);
    setSelectedId(null); setActiveSessionId(null); setActiveMsgId(null);
  ...
  function loadSession(s: Session) {
    ...
    setBusinesses([{ id: worldId, problem: s.problem, phase: 'done', msgId: aiId }]);
    setSubproblems([]); setConcepts([]); setPaperGroups({}); setRecommendations([]);
    setSelectedId(null);
    setMessages([
  ...
  async function handleSearch(problem: string) {
    ...
    setBusinesses(prev => [...prev, { id: worldId, problem, phase: 'decomposing', msgId: aiId }]);
    setActiveMsgId(aiId);
    setSelectedId(null);
    sessionSavedRef.current = false;
    ...
    try {
      ...
      const sps: SubProblem[] = (data.subproblems ?? []).map((sp: Omit<SubProblem, 'businessId'>) => ({
        ...sp,
        id: crypto.randomUUID(),
        businessId: worldId,
      }));
      setSubproblems(prev => [...prev, ...sps]);
      setBusinesses(prev => prev.map(b => b.id === worldId ? { ...b, phase: 'done' } : b));
      updateAiMsg(aiId, { phase: 'done', subproblems: sps, concepts: [], paperGroups: {} });
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : 'Error';
      setBusinesses(prev => prev.map(b => b.id === worldId ? { ...b, phase: 'error' } : b));
      updateAiMsg(aiId, { phase: 'error', error: msg });
    }
  }

  // Step 2: Transform a single sub-problem → concept
  async function handleTransformOne(msgId: string, sp: SubProblem): Promise<Concept> {
  ...
  async function handleSearchOne(msgId: string, concept: Concept) {
    ...
    const newGroups = { ...paperGroupsRef.current, [concept.subproblemId]: papers };
    ...
    if (user && accessToken && !sessionSavedRef.current && biz) {
      const bizSps = subproblemsRef2.current.filter(sp => sp.businessId === biz.id);
      const bizConcepts = allConcepts.filter(c => bizSps.some(sp => sp.id === c.subproblemId));
      const allDone = bizSps.length > 0 && bizConcepts.length === bizSps.length && bizConcepts.every(c => newGroups[c.subproblemId]?.length > 0);
      if (allDone) {
        sessionSavedRef.current = true;
  ...
  async function autoIngestAndRecommend(msgId: string, biz: Business | undefined, allConcepts: Concept[], groups: Record<string, Paper[]>) {
    updateAiMsg(msgId, { phase: 'ingesting' });
    setBusinesses(prev => prev.map(b => b.msgId === msgId ? { ...b, phase: 'ingesting' } : b));

    const latestSps = subproblemsRef2.current;
    const bizSps = biz
      ? latestSps.filter(sp => sp.businessId === biz.id && allConcepts.some(c => c.subproblemId === sp.id))
      : latestSps.filter(sp => allConcepts.some(c => c.subproblemId === sp.id));
    ...
    try {
      ...
      setBusinesses(prev => prev.map(b => b.msgId === msgId ? { ...b, phase: 'recommending' } : b));
      ...
      setBusinesses(prev => prev.map(b => b.msgId === msgId ? { ...b, phase: 'done' } : b));
  ...
  async function handleAddDomainCandidate(text: string) {
    ...
    if (!biz) return;
    const newSp: SubProblem = { id: `dc-${crypto.randomUUID()}`, text, userCreated: true, businessId: biz.id };
    setSubproblems(prev => [...prev, newSp]);
    setMessages(prev => prev.map(m =>
      m.id === activeMsgId ? { ...m, subproblems: [...(m.subproblems ?? []), newSp] } : m
    ));
    try {
  ...
  function handleDeleteNode(nodeId: string, nodeType: string) {
    if (nodeType === 'business') {
      const bizSpIds = subproblems.filter(sp => sp.businessId === nodeId).map(sp => sp.id);
      setSubproblems(prev => prev.filter(sp => sp.businessId !== nodeId));
      setConcepts(prev => prev.filter(c => !bizSpIds.includes(c.subproblemId)));
      setPaperGroups(prev => Object.fromEntries(Object.entries(prev).filter(([spId]) => !bizSpIds.includes(spId))));
      setRecommendations([]);
      setMessages(prev => {
        const biz = businessesRef.current.find(b => b.id === nodeId);
        if (!biz) return prev;
        return prev.filter(m => m.id !== biz.msgId && m.id !== biz.msgId.replace('-ai', ''));
      });
      setBusinesses(prev => {
        const next = prev.filter(b => b.id !== nodeId);
        if (next.length === 0) { setActiveMsgId(null); }
        else if (activeMsgId === businessesRef.current.find(b => b.id === nodeId)?.msgId) {
          setActiveMsgId(next[next.length - 1].msgId);
        }
        return next;
      });
      if (selectedId === nodeId || bizSpIds.includes(selectedId ?? '')) setSelectedId(null);
      return;
    }

    const patchMsg = (m: Message): Message => {
      if (m.role !== 'assistant') return m;
      if (nodeType === 'subproblem' || nodeType === 'domaincandidate') return {
        ...m,
        subproblems: (m.subproblems ?? []).filter(s => s.id !== nodeId),
        concepts: (m.concepts ?? []).filter(c => c.subproblemId !== nodeId),
        paperGroups: Object.fromEntries(Object.entries(m.paperGroups ?? {}).filter(([k]) => k !== nodeId)),
        recommendations: [],
      };
      if (nodeType === 'concept') {
        ...
        return {
          ...
          paperGroups: Object.fromEntries(Object.entries(m.paperGroups ?? {}).filter(([k]) => k !== spId)),
          recommendations: [],
        };
      }
      if (nodeType === 'paper') return {
        ...m,
        paperGroups: Object.fromEntries(Object.entries(m.paperGroups ?? {}).map(([k, ps]) => [k, ps.filter(p => p.paperId !== nodeId)])),
      };
      if (nodeType === 'recommend') return { ...m, recommendations: (m.recommendations ?? []).filter(r => r.paperId !== nodeId) };
      return m;
    };
    ...
    if (nodeType === 'subproblem' || nodeType === 'domaincandidate') {
      ...
      setPaperGroups(prev => { const n = { ...prev }; delete n[nodeId]; return n; });
      setRecommendations([]);
    } else if (nodeType === 'concept') {
      const spId = nodeId.replace(/^c-/, '');
      setConcepts(prev => prev.filter(c => c.subproblemId !== spId));
      setPaperGroups(prev => { const n = { ...prev }; delete n[spId]; return n; });
      setRecommendations([]);
    } else if (nodeType === 'paper') {
      setPaperGroups(prev => Object.fromEntries(Object.entries(prev).map(([k, ps]) => [k, ps.filter(p => p.paperId !== nodeId)])));
    ...
    <div className="flex h-screen overflow-hidden bg-white font-sans">
      ...
      {showAddNodeModal && (
        <AddNodeModal
          ...
          onAdd={text => { setShowAddNodeModal(false); void handleAddDomainCandidate(text); }}
        />
      )}

      <DetailPanel
        selectedId={selectedId} businesses={businesses} subproblems={subproblems} concepts={concepts}
        paperGroups={paperGroups} recommendations={recommendations}
        onClose={() => setSelectedId(null)}
      />

      <GraphPanel
        businesses={businesses} subproblems={subproblems} concepts={concepts}
        paperGroups={paperGroups} recommendations={recommendations}
        selectedId={selectedId} onSelect={setSelectedId}
        activeMsgId={activeMsgId ?? undefined} onTransformSp={handleTransformOne}
        onDeleteNode={handleDeleteNode}
        onAddNodeForBusiness={bizId => { setActiveMsgId(businesses.find(b => b.id === bizId)?.msgId ?? activeMsgId ?? null); setShowAddNodeModal(true); }}
      />
